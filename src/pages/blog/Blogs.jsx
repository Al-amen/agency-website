     ...
     <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10'>
      {
        blogs.map((blog,index)=>(
          <div key={blog.id} className='bg-white shadow-md rounded-lg voerflow-hidden coursor-pointer'>
            <img src={blog.image} alt={blog.title} className='w-full h-48 object-cover hover:scale-105 transition transform' />

            {/* content */}
            <div className='p-5'>
              <p className='text-sm text-blue-500 front-medium uppercase mb-2'>{blog.category}</p>
